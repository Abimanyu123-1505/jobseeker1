...
class SwipeCard {
    ...
    handleEnd(e) {
        ...
        // If clientX/clientY are undefined, fallback to last known
        if (typeof clientX !== "number") clientX = this.currentX;
        if (typeof clientY !== "number") clientY = this.currentY;
    ...
    showIndicator(deltaX, deltaY) {
        ...
        this.indicator.classList.remove('right', 'left', 'up', 'show');
        if (absX > 30 && absX > absY) {
            this.indicator.classList.add('show');
            if (deltaX > 0) {
                this.indicator.classList.add('right');
                this.indicator.innerHTML = '<i class="fas fa-heart"></i>';
            } else {
                this.indicator.classList.add('left');
                this.indicator.innerHTML = '<i class="fas fa-times"></i>';
            }
        } else if (absY > 30 && absY > absX && deltaY < 0) {
            this.indicator.classList.add('show', 'up');
...
class JobCardManager {
                ...
                <div class="job-title-section">
                    <div class="company-info">
                        <img src="${job.company.logo || ''}" alt="${job.company.name || ''}" class="company-logo"
                             onerror="this.src='https://via.placeholder.com/60x60/4F46E5/FFFFFF?text=${job.company && job.company.name ? job.company.name.charAt(0) : 'C'}'">
                        <div>
            ...
            <div class="job-content">
                <div class="job-details">
                    <div class="detail-item">
                        ...
                        <span>${job.location && job.location.city ? job.location.city : ''}, ${job.location && job.location.state ? job.location.state : ''}${job.location && job.location.isRemote ? ' (Remote)' : ''}</span>
                    </div>
                    <div class="detail-item">
                    ...
                    <div class="detail-item">
                        ...
                        <span>${job.company && job.company.size ? job.company.size : ''}</span>
                    </div>
                </div>
    ...
    enableActionButtons(enabled) {
        ...
        [rejectBtn, saveBtn, applyBtn].forEach(btn => {
            if (btn) btn.disabled = !enabled;
        });
    }

    swipeLeft() {
        if (this.activeCard && !this.isAnimating) {
            this.isAnimating = true;
            this.enableActionButtons(false);
            this.activeCard.swipeLeft();
        }
    }
